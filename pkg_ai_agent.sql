CREATE OR REPLACE PACKAGE pkg_ai_agent AS
    -- URLs y dimensiones fijas para Gemini
    ...
    C_GEMINI_CHAT_URL  CONSTANT VARCHAR2(500) := 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent';
    C_EMBED_DIMS       CONSTANT NUMBER        := 3072;

    /**
     * Obtiene la representación vectorial (embedding) de un texto utilizando la API de Gemini.
     */
    ...
    /**
     * Responde a una pregunta utilizando RAG y el modelo Gemini.
     */
    ...
    /**
     * CALLBACK DE RENDERING (Nativo de APEX 26.1.0+)
     * Adaptado a la nueva firma de tipo PROCEDURE con 4 parámetros de APEX 26.1+ para evitar el error PLS-00306.
     */
    ...
    /**
     * CALLBACK AJAX DEL CONTROLADOR (Nativo de APEX 26.1.0+)
     * Adaptado a la nueva firma de tipo PROCEDURE con 4 parámetros de APEX 26.1+ para evitar el error PLS-00306.
     */
...
CREATE OR REPLACE PACKAGE BODY pkg_ai_agent AS

FUNCTION get_embedding(p_texto IN VARCHAR2, p_api_key IN VARCHAR2) RETURN VECTOR IS
    v_request   CLOB;
    v_response  CLOB;
    v_arr_str   CLOB;
    v_embedding VECTOR;
...
BEGIN
    -- Validar de forma preventiva que la API Key de Gemini no esté vacía
    IF p_api_key IS NULL THEN
        RAISE_APPLICATION_ERROR(-20003, 'La API Key de Gemini está vacía o es nula. Por favor, ingrese su API Key en la pestaña de Atributos de la región en el Page Designer.');
    END IF;
...
BEGIN
    ...
    LOOP
        ...
        v_contexto := v_contexto || '### DOCUMENTO: ' || r_rag.titulo || CHR(10) || 
                                    r_rag.contenido || CHR(10) || '---' || CHR(10);
    END LOOP;
    CLOSE c_rag;

    v_prompt := '=== CONTEXTO DE CONOCIMIENTO (RAG) ===' || CHR(10) || 
                NVL(v_contexto, 'Sin información contextual relevante disponible.') || CHR(10) || 
                '=== PREGUNTA DEL USUARIO ===' || CHR(10) || p_pregunta;
    ...
    v_response := APEX_WEB_SERVICE.MAKE_REST_REQUEST(
        p_url         => C_GEMINI_CHAT_URL || '?key=' || p_api_key,
        p_http_method => 'POST',
        p_body        => v_request
    );
...
-- ============================================================
-- IMPLEMENTACIÓN DE CALLBACKS DEL PLUGIN (Consolidados para APEX 26.1+)
-- ============================================================
...
PROCEDURE e_render_chat (
    ...
    v_bot_name           VARCHAR2(100) := NVL(p_region.attributes.get_varchar2('bot_name'), 'Mendobot AI');
    v_welcome_msg        VARCHAR2(500) := NVL(p_region.attributes.get_varchar2('welcome_message'), '¡Hola! ¿En qué puedo ayudarte hoy?');
    v_css_override       CLOB          := p_region.attributes.get_varchar2('custom_css');
    v_avatar_url         VARCHAR2(500) := p_region.attributes.get_varchar2('bot_avatar_url');
    v_primary_color      VARCHAR2(30)  := p_region.attributes.get_varchar2('primary_color');
    
    v_avatar_src         VARCHAR2(1000);
...
BEGIN
    ...
    IF v_avatar_url IS NOT NULL THEN
        v_avatar_src := apex_escape.html_attribute(v_avatar_url);
    ELSE
        v_avatar_src := p_plugin.file_prefix || 'mendobot-logo.png';
    END IF;
    
    v_first_letter := UPPER(SUBSTR(v_bot_name, 1, 1));
    v_fallback_svg := 'data:image/svg+xml;utf8,<svg xmlns=%27http://www.w3.org/2000/svg%27 width=%2732%27 height=%2732%27 viewBox=%270 0 32 32%27><circle cx=%2716%27 cy=%2716%27 r=%2714%27 fill=%27' || NVL(REPLACE(v_primary_color, '#', '%23'), '%230066cc') || '%27/><text x=%2716%27 y=%2721%27 font-size=%2715%27 fill=%27white%27 text-anchor=%27middle%27 font-family=%27sans-serif%27 font-weight=%27bold%27>' || v_first_letter || '</text></svg>';
    ...
    IF v_css_override IS NOT NULL THEN
        sys.htp.prn('<style id="mendobot-css-override-' || v_escaped_region_id || '">');
        sys.htp.prn(v_css_override);
        sys.htp.prn('</style>');
    END IF;

    sys.htp.prn('<div id="mendobot-chat-container-' || v_escaped_region_id || '" class="mendobot-chat-wrapper js-apex-region" data-apex-region-id="mendobot-chat-container-' || v_escaped_region_id || '">');
    ...
    IF v_avatar_url IS NOT NULL THEN
        sys.htp.prn('    <img src="' || v_avatar_src || '" alt="' || apex_escape.html(v_bot_name) || '" style="width:28px;height:28px;border-radius:50%;object-fit:cover;" onerror="this.style.display=''none'';this.nextElementSibling.style.display=''block'';"><svg style="display:none" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>');
    ELSE
    ...
    sys.htp.prn('          <img src="' || v_avatar_src || '" alt="Logo ' || apex_escape.html(v_bot_name) || '" class="mendobot-avatar-logo" onerror="this.src=''' || v_fallback_svg || ''';">');
    sys.htp.prn('        </div>');
    sys.htp.prn('        <div class="mendobot-header-title">');
    sys.htp.prn('          <h3>' || apex_escape.html(v_bot_name) || '</h3>');
    sys.htp.prn('          <span class="mendobot-online-status"><span class="status-dot"></span>En línea</span>');
    sys.htp.prn('        </div>');
    sys.htp.prn('      </div>');
...
PROCEDURE e_ajax_chat (
    ...
    v_coleccion      CONSTANT VARCHAR2(30) := 'MENDOBOT_SESSION_MEMORIA';
...
BEGIN
    ...
    -- =====================================================
    -- RECUPERACIÓN DE ATRIBUTOS: Probar múltiples claves
    -- APEX usa 'ATTRIBUTE_01' (mayúsculas) por defecto.
    -- También probamos nombres personalizados en minúsculas.
    -- =====================================================
    v_api_key      := p_region.attributes.get_varchar2('api_key');
    ...
    v_bot_name     := NVL(p_region.attributes.get_varchar2('bot_name'), 'Mendobot AI');
    v_bot_role     := NVL(p_region.attributes.get_varchar2('bot_role'), 'Asistente virtual inteligente.');
    v_bot_tone     := NVL(p_region.attributes.get_varchar2('bot_tone'), 'Profesional y amable');
    v_cat_item_name := p_region.attributes.get_varchar2('category_item');
    v_license_key  := p_region.attributes.get_varchar2('license_key');
    ...
    -- =====================================================
    v_debug_msg := '';
    IF v_api_key IS NULL THEN v_debug_msg := v_debug_msg || 'API Key (Attr 01), '; END IF;
    IF v_tabla IS NULL THEN v_debug_msg := v_debug_msg || 'Tabla RAG (Attr 02), '; END IF;
    IF v_col_id IS NULL THEN v_debug_msg := v_debug_msg || 'Col ID (Attr 03), '; END IF;
    IF v_col_embed IS NULL THEN v_debug_msg := v_debug_msg || 'Col Embedding (Attr 04), '; END IF;
    IF v_col_cont IS NULL THEN v_debug_msg := v_debug_msg || 'Col Contenido (Attr 05), '; END IF;
    IF v_col_doc IS NULL THEN v_debug_msg := v_debug_msg || 'Col Titulo (Attr 06), '; END IF;

    IF v_debug_msg IS NOT NULL THEN
        HTP.PRN('{"success": false, "respuesta": "Error de Configuración: Los siguientes atributos del plugin están vacíos: ' || apex_escape.json(RTRIM(v_debug_msg, ', ')) || '. Verifique la configuración de Custom Attributes en Shared Components > Plugins y que cada atributo tenga un valor en la región del Page Designer."}');
        RETURN;
    END IF;

    -- PASO A: VALIDACIÓN SAAS "PHONE-HOME" (DESHABILITADO PARA VERIFICACIÓN)
    /*
    BEGIN
    ...
    BEGIN
        ...
        v_lic_response := APEX_WEB_SERVICE.MAKE_REST_REQUEST(
            p_url         => 'https://licensing.mendobot.com/api/v1/validate',
            p_http_method => 'POST',
            p_body        => '{"license_key":"' || NVL(v_license_key, 'DEMO') || '", "domain":"' || v_host || '"}'
        );
        
        IF v_lic_response LIKE '%"valid":false%' THEN
            HTP.PRN('{"success": false, "respuesta": "Error de Licencia (Mendobot SaaS): Su clave de licencia comercial no está autorizada para el dominio actual (' || v_host || '). Póngase en contacto con soporte."}');
            RETURN;
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            APEX_DEBUG.WARN('Mendobot SaaS - El servidor de licenciamiento central no está disponible. Operando en modo de contingencia: ' || SQLERRM);
    END;
    */
    NULL; 

    -- PASO B: RECUPERACIÓN DE MEMORIA SESIÓN (APEX_COLLECTION)
    ...
    -- PASO C: ENSAMBLAR PROMPT DEL SISTEMA CON REGLAS DE NEGOCIO ESTRICTAS
    v_system_prompt := 
        '=== IDENTIDAD DEL BOT ===' || CHR(10) ||
        'Nombre: ' || v_bot_name || CHR(10) ||
        'Rol y Objetivo: ' || v_bot_role || CHR(10) ||
        'Tono de la conversación: ' || v_bot_tone || CHR(10) || CHR(10) ||
        '=== [REGLAS CRÍTICAS DE RESPUESTA] ===' || CHR(10) ||
        '- Responde basándote ÚNICAMENTE en el CONTEXTO DE CONOCIMIENTO (RAG) adjunto.' || CHR(10) ||
        '- Si el contexto proporcionado no contiene la información para responder la pregunta, di textualmente: "Disculpe, pero no tengo información oficial en mi base de conocimiento para responder a su consulta." y detente inmediatamente.' || CHR(10) ||
        '- PROHIBIDO utilizar palabras introductorias innecesarias como "Basado en...", "De acuerdo con el documento...", etc.' || CHR(10) ||
        '- PROHIBIDO adivinar, asumir o referenciar información externa no provista en el RAG.' || CHR(10) ||
        '- Sé directo y mantén una estructura conversacional limpia.' || CHR(10) || CHR(10) ||
        '=== MEMORIA DE CONVERSACIÓN ACTIVA (Límite: ' || v_limite_chats || ' chats) ===' || CHR(10) ||
        NVL(v_historial_clob, 'Sin interacciones previas.') || CHR(10) || '---';

    IF v_cat_item_name IS NOT NULL THEN
        v_cat_val := V(v_cat_item_name);
    END IF;

    -- PASO D: LLAMAR AL MOTOR DE BÚSQUEDA VECTORIAL E IA (RAG)
    ...
    -- PASO E: PERSISTENCIA EN MEMORIA DE SESIÓN Y AUDITORÍA
    ...
    -- PASO F: RETORNAR RESPUESTA JSON SEGURA AL CLIENTE
    BEGIN 
        apex_json.free_output; 
    EXCEPTION 
